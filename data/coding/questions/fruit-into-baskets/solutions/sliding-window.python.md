...
`counts` is a live tally of the current window, so `len(counts)` is exactly the number of baskets in use. The `while` loop only runs when there are three types, and it always restores the two-type invariant before the length is recorded, so every measured window is legal. Because `left` never moves backward, the answer is the widest valid window ever seen.
