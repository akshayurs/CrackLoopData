Start from every index and extend a window rightward, accumulating the sum until it reaches `target`. The moment a start position produces a qualifying window, record its length and stop — no shorter window can begin at that same start.
