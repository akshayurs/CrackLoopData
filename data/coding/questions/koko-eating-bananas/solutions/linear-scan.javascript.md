...
`Math.ceil(p / k)` is exactly the hours Koko spends on one pile, because she cannot carry leftover eating into the next hour. Summing gives the total time at speed `k`. Since that total is non-increasing in `k`, scanning upward finds the minimum feasible speed first. Speed `maxPile` always works (one pile per hour), so the loop is guaranteed to return.
