...
The answer is a contiguous window, so the only real choice is where its edges land. Between the two current boundaries, the farther one is strictly worse than the closer one and worse than everything between them, so it is safe to drop. The strict `>` makes equal distances discard the right end and keep the smaller-valued left element. When the window holds exactly `k` items it is collected in order, already ascending.

## Complexity

- Time: O(n - k) — each iteration removes one element until k remain.
- Space: O(1) — only two pointers, ignoring the output list.
