The same direct reading in C++: at every node, either rob it and add whatever the grandchildren yield, or skip it and take the best each child can offer on its own. Recurse on both branches and keep the larger.
...
public:
    int rob(TreeNode* root) {
        ...
        return std::max(robThis, skipThis);
    }
};
```
