...
Capturing `queue.size()` before the inner loop fixes how many nodes belong to the current level, so each `while` iteration consumes exactly one level and adds one to `depth`. Children offered during the loop wait until the next round. The outer loop runs once per level, so the final count equals the number of levels — the maximum depth.
