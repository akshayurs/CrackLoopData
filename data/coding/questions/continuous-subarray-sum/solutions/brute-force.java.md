...
Growing the sum incrementally instead of re-adding a slice each time keeps the inner step O(1). We use a `long` accumulator so the sum can't overflow while it builds up.
