The simplest way to bound the target is to walk the vector from left to right and remember the first index where it appears and the most recent index where it appears. The first match fixes the left boundary; the last match seen by the end of the scan fixes the right boundary.
