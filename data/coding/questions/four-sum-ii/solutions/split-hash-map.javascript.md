Split the four arrays into two halves. Precompute every pairwise sum `a[i] + b[j]` and tally how often each sum occurs in a map. Then, for every pair from `c` and `d`, the tuple sums to zero exactly when `a[i] + b[j] === -(c[k] + d[l])`, so you just look up how many left-half pairs produce that complement.
