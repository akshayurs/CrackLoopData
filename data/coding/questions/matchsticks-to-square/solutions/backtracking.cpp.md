...
public:
    bool canFormSquare(vector<int>& matchsticks) {
        long total = accumulate(matchsticks.begin(), matchsticks.end(), 0L);
...
private:
    bool backtrack(vector<int>& sticks, int i, vector<long>& sides, long side) {
...
`sides` tracks the running length of each of the four sides. Placing a stick on a side that would push it past `side` is never useful, so that branch is skipped. When every stick has been placed (`i == sticks.size()`), the square is valid only if all four running totals equal `side` exactly. Undoing `sides[s] -= sticks[i]` after a failed recursive call restores the state so the next side can be tried — standard backtracking.
