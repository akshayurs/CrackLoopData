...
Each step halves the search space, so the window shrinks logarithmically. Compute the midpoint as `lo + ((hi - lo) >> 1)` to keep the intent clear and avoid overflow.
