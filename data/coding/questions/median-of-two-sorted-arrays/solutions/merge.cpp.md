...
The merge preserves sorted order because at every step the smallest unused element across both arrays sits at one of the two pointers, and we always take it. After the loop, one array is exhausted and the leftover tail of the other is already in order, so copying it keeps the result sorted. Indexing the middle of a sorted array is the definition of the median.
