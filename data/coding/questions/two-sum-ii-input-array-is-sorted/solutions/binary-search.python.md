...
For each `i`, the partner (if it exists) is a single value `target - numbers[i]`. Because the suffix `numbers[i+1:]` is sorted, binary search locates that value in logarithmic time. Searching only to the right of `i` guarantees `index1 < index2` and prevents reusing the same element.
