...
The base case is the empty tree, which is its own mirror, so we simply return `null` and let the recursion unwind.
...
Each call fully mirrors the subtree rooted at `root`: the recursive calls return already-inverted left and right subtrees, and we reattach them on the opposite sides. Because every node is visited exactly once and its children are exchanged, the entire tree ends up mirrored. Leaves bottom out at the `null` base case.
