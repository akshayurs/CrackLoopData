...
Do one DFS that returns the height of each subtree as usual, but before returning, check whether `left height + right height` beats the best diameter seen so far and record it in a member shared across calls.
