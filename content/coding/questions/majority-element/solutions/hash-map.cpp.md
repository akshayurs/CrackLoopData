The quadratic cost of the brute force comes from recounting the same value again and again. Tally every value's frequency in a single pass instead: an `unordered_map` from value to running count means each element is touched once.
