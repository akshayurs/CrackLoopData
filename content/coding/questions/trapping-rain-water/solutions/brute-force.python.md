...
Water above column `i` can only rise as high as the lower of its two surrounding walls, `min(left_max, right_max)`. Subtracting `height[i]` gives the depth at that column, and this is always non-negative because both maxima include `height[i]` itself. Summing over every column yields the total volume.
