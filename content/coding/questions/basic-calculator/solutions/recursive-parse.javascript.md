...
When you hit `(`, recurse to evaluate the inner expression and treat its return value as the next `num`; when you hit `)`, fold the pending number in and return. A shared cursor `pos` tells every frame where scanning currently sits, so it resumes past the matching `)`.
