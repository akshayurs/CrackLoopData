...
private:
    ...
    bool dfs(TrieNode* node, const string& word, size_t i) {
...
Each `TrieNode` groups the 26 possible next letters, so words sharing a prefix share the same chain of nodes. `dfs` advances one query character per call: a concrete letter narrows the search to exactly one child, while `.` fans out over every non-null child and returns as soon as any branch reaches the end of `word` on a node flagged as a word ending. Returning `false` on a `nullptr` node prunes dead branches immediately instead of exploring further. Because dots only appear a handful of times per query (bounded by the constraints), the fan-out stays cheap in practice even though it is exponential in the dot count.
