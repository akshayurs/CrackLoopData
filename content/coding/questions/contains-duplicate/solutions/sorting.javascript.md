...
Sorting groups equal values into contiguous runs. If any value repeats, at least two copies end up adjacent, so a single neighbour comparison across the sorted array is enough to catch it. If no adjacent pair is equal, every value is distinct. Note the numeric comparator — JavaScript's default sort is lexicographic and would misorder numbers.
