...
For day `i` the inner loop visits days `i+1, i+2, ...` in order, so the first `j` with a strictly greater temperature is the nearest warmer day. Recording `j - i` gives the wait; breaking immediately guarantees it is the *closest* such day. Days with no warmer future keep their initial `0`.
