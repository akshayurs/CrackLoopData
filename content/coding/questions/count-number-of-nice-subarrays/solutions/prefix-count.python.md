...
`odds` is the number of odd values in `nums[0..i]`. A subarray `nums[j+1..i]` holds exactly `k` odds when `odds - prefix(j) == k`, i.e. `prefix(j) == odds - k`. `count` records how many earlier prefixes had each value, so `count[odds - k]` is exactly the number of valid left boundaries for the current right end. Recording the current prefix after the lookup keeps boundaries strictly earlier.
