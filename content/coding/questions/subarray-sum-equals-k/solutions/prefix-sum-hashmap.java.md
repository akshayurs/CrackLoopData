...
So carry one running sum and a `HashMap` that counts how many times each prefix value has occurred. At each element, look up how many earlier prefixes equal `running - k`: each one marks a distinct subarray ending here that sums to `k`. Seeding the map with `0 -> 1` lets a subarray that starts at index 0 be counted.
