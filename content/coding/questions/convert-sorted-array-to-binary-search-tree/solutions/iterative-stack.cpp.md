Recursion is the natural fit here, but it ties one stack frame to every pending subtree. You can get the same result by managing that "pending work" yourself with an explicit stack of frames instead of letting the call stack do it.
