...
`height` returns the longest root-to-leaf distance in a subtree, so `abs(height(left) - height(right))` is exactly the imbalance at the current node. Checking it here guarantees the local rule; recursing into both children extends the guarantee to every node. If any node fails, the `and` short-circuits and `False` propagates up.
