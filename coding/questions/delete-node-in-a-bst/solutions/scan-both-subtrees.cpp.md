...
Once the target node is found, handle its children. Zero or one child is easy — return whichever child exists (or `nullptr`). Two children means copying up the in-order successor's value (the leftmost node of the right subtree) and then recursively deleting that successor's value from the right subtree, which is itself a smaller instance of the same problem.
