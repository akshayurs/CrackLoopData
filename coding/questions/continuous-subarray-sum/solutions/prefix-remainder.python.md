...
`first_seen[r]` holds the earliest index where the prefix sum had remainder `r`. If the current prefix has that same remainder `r`, the block strictly after the stored index up to `i` sums to a multiple of `k`. We only accept it when the index gap is at least two, enforcing the length requirement. Storing only the first occurrence guarantees the widest possible gap, so any qualifying window is found.
