...
`(p + k - 1) / k` is integer ceiling division — exactly the hours Koko needs for one pile, since she cannot carry leftover eating into the next hour. The running sum uses `long` to avoid overflow when many large piles accumulate. The total is non-increasing in `k`, so the upward scan returns the minimum feasible speed first; speed `maxPile` always works.
