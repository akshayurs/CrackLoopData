...
`reverse_list(head.next)` returns the head of the already-reversed remainder while `head.next` still refers to the node that was directly after `head` — now the tail of that reversed portion. Setting `head.next.next = head` appends the current node after it, and `head.next = None` prevents a cycle (the old head becomes the new tail). `new_head` is unchanged as the stack unwinds, so the deepest node is returned all the way up.
