...
`reverseList(head.next)` returns the head of the already-reversed remainder while `head.next` still refers to the node that was directly after `head` — now the tail of that reversed portion. Setting `head.next.next = head` appends the current node after it, and `head.next = null` prevents a cycle (the old head becomes the new tail). `newHead` is unchanged as the stack unwinds, so the deepest node is returned all the way up.
