There is no need to store or rescan the whole path — the only fact that matters at any node is the single largest value seen on the way down. Carry that one number as an argument through the recursion instead of rebuilding it from a vector every time.
