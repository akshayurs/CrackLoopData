Each group only ever needs to answer one question about a digit: "have I seen you before?" Since there are just nine possible digits, that membership set fits in nine bits of a single `int` — bit `d-1` is set once digit `d` has appeared. This replaces every `HashSet` with one primitive integer.

Keep one `int` per row, column, and box. For a digit `v`, form `bit = 1 << (v - '1')`; if that bit is already lit in any of the three masks, the digit repeats. Otherwise OR it into all three. One pass, and no boxing or hashing at all.
