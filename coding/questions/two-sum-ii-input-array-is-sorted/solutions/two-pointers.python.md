...
At any point, `numbers[lo]` is the smallest still-usable value and `numbers[hi]` the largest. If their sum is below `target`, pairing `numbers[hi]` with anything to the left of `lo` would only be smaller, so `numbers[lo]` cannot reach the target and is safely skipped. The symmetric argument retires `numbers[hi]` when the sum is too large. Every move eliminates exactly one element, and the guaranteed pair is met when the pointers cross it.
