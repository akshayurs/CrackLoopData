...
The dummy node removes the special case of deleting the head — `prev` is never `None`, so `prev.next = curr.next` always works, including when the first node itself must go. `prev` only advances when the current node is kept, so it always trails the most recent surviving node, and `curr` advances every iteration regardless, guaranteeing termination after one full pass.
