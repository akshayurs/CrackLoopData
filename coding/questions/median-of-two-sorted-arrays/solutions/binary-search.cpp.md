...
public:
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
        ...
        int m = A.size(), n = B.size(), total = m + n, half = (total + 1) / 2;
        int lo = 0, hi = m;
        while (lo <= hi) {
            int i = (lo + hi) / 2, j = half - i;
            long leftA = i > 0 ? A[i - 1] : LONG_MIN;
            long rightA = i < m ? A[i] : LONG_MAX;
            long leftB = j > 0 ? B[j - 1] : LONG_MIN;
            long rightB = j < n ? B[j] : LONG_MAX;
